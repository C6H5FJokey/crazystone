...
execute if score res2 int matches 1 if block ~ ~ ~ #jokeygame:furnace if data storage jokeygame:io temp2{Slot: 2b} run scoreboard players set res int 0
# 开始检查
...
execute if score res2 int matches 1 if block ~ ~ ~ #jokeygame:furnace if score res int matches 0 if score res1 int matches 0 run return fail
# 截断
...
execute if score res2 int matches 1 if block ~ ~ ~ #jokeygame:furnace run data modify storage jokeygame:io temp2 set value {}
# 看上去很奇怪，但是合理的，如果熔炉里只有产物，那么slot[0]一定是slot:2b, 否则无论如何第一轮如果可能一定测过slot:0b了，如果没有则第一轮测的是1b，这里再强行测1b后，一定是检测过两个槽能合并哪个槽了。
execute if score res2 int matches 1 if block ~ ~ ~ #jokeygame:furnace run data modify storage jokeygame:io temp2 set from block ~ ~ ~ Items[{Slot: 1b}]
execute if score res2 int matches 1 if block ~ ~ ~ #jokeygame:furnace run function jokeygame:slot/pipe_slot/_add_last_slot_predicate
...
execute if block ~ ~ ~ #jokeygame:furnace if items block ~ ~ ~ container.1 * if items entity e-0-0-0-0 weapon.mainhand #jokeygame:fuel unless score res int matches 1 run return fail
