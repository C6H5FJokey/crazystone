...
execute if items block ~ ~ ~ container.* * run scoreboard players set res int 1
execute if score res int matches 1 run data modify storage jokeygame:io temp2 set value {}
# 正常容器判定末尾物品
execute if score res int matches 1 if block ~ ~ ~ #jokeygame:containers run data modify storage jokeygame:io temp2 set from block ~ ~ ~ Items[-1]
# 熔炉先判定一次有的槽
execute if score res int matches 1 if block ~ ~ ~ #jokeygame:furnace run scoreboard players set res2 int 1
execute if score res2 int matches 1 run data modify storage jokeygame:io temp2 set from block ~ ~ ~ Items[{Slot: 0b}]
function jokeygame:slot/pipe_slot/_add_last_slot_predicate
# 熔炉第二次判定
